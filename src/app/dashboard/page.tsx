    ...
    return (
      <div className="min-h-screen bg-bee-gradient flex items-center justify-center">
        <div className="text-center">
          ...
          <h1 className="text-4xl font-bold text-amber-900 mb-4">Session Activated! 🐝</h1>
      ...
      <div className="absolute inset-0 opacity-20">
        <div className="grid grid-cols-8 gap-4 p-8 transform rotate-12 scale-150">
          {Array.from({ length: 64 }).map((_, i) => (
            <div key={i} className="w-8 h-8 border-2 border-amber-400/60 transform rotate-45" />
          ))}
        </div>
      </div>
        ...
        <main className="flex-1 px-4 md:px-8 pb-12">
          <div className="max-w-7xl mx-auto space-y-12">
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-5 flex flex-col items-center space-y-6">
                ...
                <p className="text-lg text-amber-800 text-center max-w-sm leading-relaxed">
                  <span className="font-semibold">Scan this code</span> with your phone to begin your focus ritual.
                </p>
                {/* Status */}
                {isPhoneConnected ? (
                  <div className="bg-green-50 border-2 border-green-200 rounded-2xl p-6 w-full text-center">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      ...
                      <p className="text-green-800 font-semibold">Phone Connected!</p>
                    </div>
                    {ritualStep && <p className="text-green-700">{ritualStep}</p>}
                  </div>
                ) : (
                  <div className="bg-white/90 backdrop-blur-sm border-2 border-amber-200 rounded-2xl p-6 w-full text-center">
                    <div className="flex items-center justify-center gap-2 mb-2">
                      ...
                      <p className="text-amber-800 font-semibold">Ready to Focus</p>
                    </div>
                    <p className="text-amber-600 text-sm">Scan the QR code to connect your phone</p>
                  </div>
                )}
              </div>
              ...
              <div className="lg:col-span-7 space-y-8">
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-yellow-50 rounded-2xl p-6 border border-amber-200 text-center">
                    ...
                    <div className="text-2xl font-bold text-amber-900">{stats.totalSessions}</div>
                  ...
                  <div className="bg-green-50 rounded-2xl p-6 border border-green-200 text-center">
                    ...
                    <div className="text-2xl font-bold text-green-800">{formatTime(stats.totalFocusTime)}</div>
                  ...
                  <div className="bg-orange-50 rounded-2xl p-6 border border-orange-200 text-center">
                    ...
                    <div className="text-2xl font-bold text-orange-800">{stats.streakDays}</div>
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-amber-200">
                  ...
                  <div className="flex items-end gap-2 h-32 mb-4">
                    {stats.weeklyFocus.map((minutes, index) => {
                      const height = (minutes / Math.max(...stats.weeklyFocus)) * 100;
                        ...
                        <div key={index} className="flex-1 flex flex-col items-center gap-1">
                          <div className="w-full bg-amber-100 rounded-t-xl relative">
                            <div
                              ...
                              style={{ height: `${height}%` }}
                            >
                              <span className="text-white text-xs font-bold mb-1">{minutes}m</span>
                            </div>
                          </div>
                          <span className="text-amber-700 text-xs font-bold">{days[index]}</span>
                        </div>
                      );
                    })}
                  </div>
                  <div className="bg-amber-50 rounded-xl p-4 text-center">
                    <p className="text-amber-800 font-semibold">
                      Total this week: {formatTime(stats.weeklyFocus.reduce((a, b) => a + b, 0))} 🎉
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-white/90 backdrop-blur-sm rounded-3xl p-6 shadow-xl border border-amber-200">
                  ...
                  <div className="space-y-4">
                    <div className="flex justify-between items-center p-4 bg-amber-50 rounded-xl">
                      <span className="text-amber-800 font-medium">Favorite Mode</span>
                      <span className="font-bold text-amber-900 capitalize">{stats.favoriteMode.replace("-", " ")} 💛</span>
                    </div>
                    <div className="flex justify-between items-center p-4 bg-green-50 rounded-xl">
                      <span className="text-green-800 font-medium">Average Session</span>
                      <span className="font-bold text-green-900">{formatTime(stats.avgSessionLength)} ⏱️</span>
                    </div>
                    <div className="flex justify-between items-center p-4 bg-blue-50 rounded-xl">
                      <span className="text-blue-800 font-medium">Completion Rate</span>
                      <span className="font-bold text-blue-900">{stats.completedSessions}/{stats.totalSessions} ✅</span>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-r from-amber-50 to-yellow-50 border-2 border-amber-200 rounded-3xl p-8 shadow-lg text-center">
              <h3 className="text-xl md:text-2xl font-bold text-amber-900 mb-6">Your Daily Bee Wisdom 🌻</h3>
              <p className="text-lg md:text-xl text-amber-700 leading-relaxed">
                "Remember, even the busiest bees take time to rest between flights! You're doing incredible work building these focus habits. I'm so proud of how far you've come, and I can't wait to see what amazing things you'll accomplish next! Keep being awesome! 🌟"
              </p>
            </div>
          </div>
        </main>
